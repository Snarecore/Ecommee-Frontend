...
interface ProductSectionTwoProps {
    productList: Product[];
    contentData: any;
}

const ProductSectionTwo: React.FC<ProductSectionTwoProps> = ({ productList, contentData }) => {
    if (!contentData?.isProductSectionTwoVisible) return null;
    ...
    return (
        <div className='max-w-screen-sm md:max-w-screen-md lg:max-w-screen-lg xl:max-w-screen-xl 2xl:max-w-screen-2xl mx-auto px-4'>
            <div className="mt-6 mb-4">
                <p className="text-4xl font-bold rounded-md p-2 text-center" 
                style={{
                    backgroundColor: contentData?.productSectionTwoBackgroundColor,
                    color: contentData?.productSectionTwoFontColor,
                }}>
                    {contentData?.productSectionTwoTitle}
                </p>
            </div>
